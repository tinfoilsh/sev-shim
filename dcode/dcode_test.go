package dcode

import (
	"math/rand/v2"
	"strings"
	"testing"

	"github.com/stretchr/testify/assert"

	"github.com/tinfoilsh/verifier/attestation"
)

func TestDcode(t *testing.T) {
	for domain, attestation := range map[string]attestation.Document{
		"example.com": {
			Format: "https://tinfoil.sh/predicate/sev-snp-guest/v1",
			Body:   "",
		},
		"empty.com": {
			Format: "https://tinfoil.sh/predicate/sev-snp-guest/v1",
			Body:   "",
		},
	} {
		t.Run(domain, func(t *testing.T) {
			domains, err := Encode(&attestation, domain)
			assert.Nil(t, err)

			for _, chunk := range domains {
				assert.True(t, strings.HasSuffix(chunk, "."+domain))
			}

			t.Logf("encoded into %d domains", len(domains))

			// Randomize domain order
			rand.Shuffle(len(domains), func(i, j int) {
				domains[i], domains[j] = domains[j], domains[i]
			})

			for _, domain := range domains {
				t.Logf("domain: %s", domain)
			}

			decoded, err := Decode(domains)
			assert.Nil(t, err)
			assert.Equal(t, attestation, *decoded)
		})
	}
}
